<template>
  ...
  <div class="container mx-auto p-4">
      ...
      <!-- Add New Item Button -->
      <div class="mb-4">
          <button @click="openAddModal" class="bg-gray-800 hover:bg-gray-900 text-white font-bold py-2 px-4 rounded">
              Add New Item
          </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    // Form state management
      ...
      removeVernacularName(index) {
          if (this.formData.vernacular_names.length > 1) {
      ...
      editItem(item) {
          ...
          this.formData = {
              ...
              vernacular_names: Array.isArray(item.vernacular_names) && item.vernacular_names.length > 0
  ...
  try {
      ...
      // Add basic fields
      formDataToSubmit.append('common_name', this.formData.common_name);
      formDataToSubmit.append('scientific_name', this.formData.scientific_name);
      formDataToSubmit.append('description', this.formData.description);
      formDataToSubmit.append('category', this.formData.category);
      formDataToSubmit.append('more_info', this.formData.more_info);

      // Append vernacular names individually
      this.formData.vernacular_names.forEach((vernacular, index) => {
          formDataToSubmit.append(`vernacular_names[${index}][name]`, vernacular.name);
          formDataToSubmit.append(`vernacular_names[${index}][place]`, vernacular.place);
      });
      ...
      else if(response.status === 400 || response.response.data.status === 400) {
          console.log(response);
      ...
      else if(error.response.status === 500 || error.response.data.status === 500) {
          console.log(response);
