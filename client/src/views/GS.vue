<template>
    <SearchLayout>
      <transition-group name="fade" appear tag="div" class="flex flex-col gap-2 w-full h-full">
        <div key="main-content" class="flex flex-col gap-7 w-full h-full">
          <h1 key="title" class="text-2xl text-primary uppercase font-bold text-center">Global Search</h1>
        ...
        <template v-else-if="filteredSuggestions.length > 0 && search.length > 0">
        ...
        <template v-else-if="(filteredSuggestions.length === 0 && search.length > 0 && !isLoading)">
  ...
  <script>
  import { getGSearchSuggestions } from '@/API/index.js'
  ...
  export default {
    ...
    computed: {
      filteredSuggestions() {
        return this.suggestions.filter(suggestion => suggestion?.name)
      }
    },
    methods: {
      async searchItem() {
        ...
        try {
          ...
          const res = await getGSearchSuggestions(this.search)
          console.log(res);
      ...
      handleSearch() {
        ...
        // commented code is for adding debouncer

        // this.debounce = setTimeout(() => {
        //   this.searchItem();
        // }, 100)

          this.searchItem();  // searchinng without debouncer
      ...
      performSearch() {
        console.log('Performing global search with:', this.search);
        this.searchItem();
      }
    }
  };
